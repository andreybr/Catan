...
{
    public class MonopolyMenu : XNACS1Rectangle
    {
            ...
            if (nCardsPicked == nCARDS_GAINED)
            {
                ...
                if (oCursor.oHitbox.Collided(oButtons1[0]))
                {
                    ...
                    for (int x = 0; x < oPlayers.Count(); x++)
                    {
                        ...
                        oPlayers[0].nCards[nCardType] += oPlayers[x].nCards[nCardType];
                        oPlayers[x].nCards[nCardType] = 0;
                    }

                    oHUD.ShowCards(oPlayers[0].nCards, oCamera);
                    bFinished = true;
                    }
                }
            }
